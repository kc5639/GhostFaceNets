...
class Train:
    def __init__(
        ...
        custom_objects.update({"NormDense": NormDense})
    ...
    def __init_model__(self, type, loss_top_k=1, header_append_norm=False):
        ...
        if type == self.softmax and model_output_layer_name != self.softmax:
            ...
            if vpl_start_iters > 0:
                ...
                arcface_logits = NormDenseVPL(batch_size, self.classes, output_kernel_regularizer, **arc_kwargs, **vpl_kwargs, dtype="float32")
            else:
                arcface_logits = NormDense(self.classes, output_kernel_regularizer, **arc_kwargs, dtype="float32")
